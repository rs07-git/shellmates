...
const ORCHESTRATOR_CMDS = {
  claude: (promptFile, bypass) => bypass
    ? `claude --dangerously-skip-permissions "$(cat ${promptFile})"`
    : `claude "$(cat ${promptFile})"`,
  gemini: (promptFile, bypass) => bypass
    ? `gemini --yolo -p "$(cat ${promptFile})"`
    : `gemini -p "$(cat ${promptFile})"`,
  codex: (promptFile, bypass) => bypass
    ? `codex --full-auto "$(cat ${promptFile})"`
...
export async function pond(options) {
  ...
  const orchBypass = (config.orchestrator_permission_mode || 'default') === 'bypass'

  if (!ORCHESTRATOR_CMDS[orchestrator]) {
    console.log(chalk.red(`\n  ✗ Orchestrator "${orchestrator}" doesn't support pond mode yet.`))
    console.log(chalk.dim('  Set orchestrator to "claude", "gemini", or "codex" in shellmates config.\n'))
    process.exit(1)
  }
  ...
  const orchCmd = ORCHESTRATOR_CMDS[orchestrator](promptFile, orchBypass)
  ...
  console.log(chalk.dim('  Orchestrator: ') + chalk.bold(orchestrator) + (orchBypass ? chalk.yellow(' (bypass)') : ''))
